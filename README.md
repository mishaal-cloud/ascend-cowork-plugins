# Ascend Plugins

Plugin marketplace for Ascend GTM — shared context and skills across Codex, Claude Cowork, and Claude Code.

## Plugins

- **ascend-gtm-operator** — surface-specific operator doctrine injected at session start plus the `ascend-doctrine` skill. Codex uses `.codex-plugin/plugin.json` and `OPERATOR-CODEX.md`; Claude retains its existing Cowork context.
  - Sub-agents (auto-discovered from `agents/`, run in Cowork only):
    - `gtm-researcher` — fast sourced research on GTM strategy, B2B SaaS benchmarks, PE portfolio intel, competitors, market sizing.
    - `marketing-analyst` — read-only pipeline and attribution analysis over live gateway data, with explicit confidence, data limits, and no fabricated metrics.

## Install

**Cowork / Claude Desktop:** Customize → Plugins → + → Add marketplace → `mishaal-cloud/ascend-cowork-plugins` (or upload the plugin folder if private-repo sync is unavailable). Then install `ascend-gtm-operator`.

**Claude Code CLI:** `/plugin marketplace add mishaal-cloud/ascend-cowork-plugins` then `/plugin install ascend-gtm-operator@ascend-plugins`. Note: Claude Code already loads `~/.claude/CLAUDE.md` — installing there duplicates context; intended surface is Cowork.

## Related infrastructure

- memZERO memory connector for claude.ai/Cowork: `https://memory-auth.ascendgtm.net/mcp` (OAuth proxy → `memory-mcp.ascendgtm.net`, repo: `mishaal-cloud/memory-oauth-proxy`)
